...
- `server/` — FastAPI backend (`server/app.py`, single file) that calls the OpenAI Responses/Chat Completions APIs. Sessions are in-memory only; permissive CORS; no auth or persistence.
...
- Never commit `server/.env`, `local.properties`, or real API keys.
